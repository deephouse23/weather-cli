export const STATE_MAPPINGS = {
  ...
  'WI': 'Wisconsin, US', 'WY': 'Wyoming, US', 'DC': 'Washington DC, US',
  
  // Canadian Provinces
  'ON': 'Ontario, CA', 'QC': 'Quebec, CA', 'BC': 'British Columbia, CA', 'AB': 'Alberta, CA',
  'MB': 'Manitoba, CA', 'SK': 'Saskatchewan, CA', 'NS': 'Nova Scotia, CA', 'NB': 'New Brunswick, CA',
  'NL': 'Newfoundland, CA', 'PE': 'Prince Edward Island, CA', 'NT': 'Northwest Territories, CA',
  'YT': 'Yukon, CA', 'NU': 'Nunavut, CA',
  
  // Country Codes
  'USA': 'United States', 'UK': 'United Kingdom', 'UAE': 'United Arab Emirates',
  'JP': 'Japan', 'FR': 'France', 'DE': 'Germany', 'AU': 'Australia', 'NZ': 'New Zealand',
  'IT': 'Italy', 'ES': 'Spain', 'MX': 'Mexico', 'BR': 'Brazil', 'IN': 'India', 'CN': 'China'
};
...
export function parseLocation(args) {
  ...
  const locationArgs = args.filter(arg => 
    !arg.startsWith('-') && 
    !['metric', 'imperial', 'celsius', 'fahrenheit', 'auto'].includes(arg)
  );
  ...
  if (parts.length >= 2) {
    ...
    if (STATE_MAPPINGS[lastPart]) {
      // Replace last part with full state/country name
      const cityParts = parts.slice(0, -1).join(' ');
      return `${cityParts}, ${lastPart}, US`;
    }
